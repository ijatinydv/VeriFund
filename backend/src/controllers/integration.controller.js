...
class IntegrationController {
   ...
   * Update project's potential score after AI processing
   * Called by AI service after processing webhook
   * @public - Called by AI service
   */
  async handleScoreUpdate(req, res) {
    try {
      const { projectId, scoreIncrease, commitMessage } = req.body;
      ...
      if (scoreIncrease === undefined || scoreIncrease === null) {
        return res.status(400).json({
          success: false,
          message: 'scoreIncrease is required'
        });
      }
      ...
      // Calculate new score
      const oldScore = project.potentialScore || 75;
      let newScore = oldScore + scoreIncrease;
      ...
      console.log(`✓ Score updated: ${oldScore} → ${newScore} (+${scoreIncrease})`);

      // TODO: Emit WebSocket event for real-time updates (optional)
      // io.to(`project:${projectId}`).emit('scoreUpdated', { projectId, oldScore, newScore, scoreIncrease });
